...
#Ejemplo 2
#Mostrar los profesores que imparten "Full Stack Python"
SELECT * FROM profesores
WHERE curso_id = (
	SELECT id
    FROM cursos
    WHERE nombre = 'Full Stack Python'
);

#Ejemplo 3
#Mostrar estudiantes que perteneces al mismo curso que Roberto 
SELECT *
FROM estudiantes
WHERE curso_id = (
	SELECT curso_id
...
#Ejemplo 4
#Mostrar estudiantes tienen una edad superior al promedio del curso
SELECT *
FROM estudiantes
WHERE edad > (
	SELECT AVG(edad)
...
#==Ejecutar
CALL agregar_estudiante('Camila', 23, 2);
...
BEGIN
    ...
    INSERT INTO profesores_eliminados (
        nombre,
        fecha_eliminacion
    )
    VALUES (
